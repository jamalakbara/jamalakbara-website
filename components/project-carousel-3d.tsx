...
function ProjectCard({
  ...
  // Card dimensions
  const cardWidth = Math.min(viewport.width * 0.5, 6)
  ...
  useFrame(() => {
    ...
    // Show: active (0), 1 neighbor on each side, hide rest
    const isActive = distanceFromActive === 0
    const isNeighbor = absDistance === 1
    const isVisible = absDistance <= 1

    // Positioning - active centered, neighbors on sides, others off-screen
    const spacing = cardWidth * 0.7
    const targetX = isVisible ? distanceFromActive * spacing : (distanceFromActive > 0 ? 15 : -15)
    const targetZ = isActive ? 0.5 : (isNeighbor ? -1.5 : -5)
    const targetScale = isActive ? 1 : (isNeighbor ? 0.7 : 0.3)
    const targetOpacity = isActive ? 1 : (isNeighbor ? 0.25 : 0)
    const targetBlur = isActive ? 0 : (isNeighbor ? 1 : 0)
    const targetRotationY = isNeighbor ? distanceFromActive * 0.15 : 0
...
export function ProjectCarousel3D() {
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden pt-20">
      ...
      <div className="absolute left-8 md:left-16 bottom-20 md:bottom-16 z-10 max-w-md">
        <AnimatePresence mode="wait">
          ...
          >
            <span className="inline-block px-3 py-1 mb-3 rounded-full border border-white/20 text-xs font-mono uppercase tracking-wider text-white/60">
              {currentProject.category}
            </span>
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-2 tracking-tight">
              {currentProject.title}
            </h2>
            <p className="text-sm text-white/40 mb-4">{currentProject.year}</p>

            <div className="flex flex-wrap gap-2 mb-5">
              {currentProject.tech?.slice(0, 3).map((tech, i) => (
                <span key={i} className="text-xs text-white/50 bg-white/5 px-2 py-1 rounded">{tech}</span>
              ))}
            </div>
      ...
      <div className="absolute right-8 md:right-16 bottom-20 md:bottom-16 z-10">
      ...
      <div className="absolute top-24 left-8 md:left-16 z-10">
        <span className="text-xs font-mono uppercase tracking-widest text-white/40">Selected Works</span>
      </div>
      ...
      <div className="absolute left-8 md:left-16 top-1/2 -translate-y-1/2 z-20">
        <button
          ...
          className={`w-12 h-12 md:w-14 md:h-14 rounded-full border flex items-center justify-center transition-all duration-300 ${activeProject === 0
      ...
      <div className="absolute right-8 md:right-16 top-1/2 -translate-y-1/2 z-20">
        <button
          ...
          className={`w-12 h-12 md:w-14 md:h-14 rounded-full border flex items-center justify-center transition-all duration-300 ${activeProject === maxProjects - 1
            ? 'border-white/10 text-white/20 cursor-not-allowed'
            : 'border-white/30 text-white/70 hover:border-white hover:text-white hover:bg-white/10'
            }`}
